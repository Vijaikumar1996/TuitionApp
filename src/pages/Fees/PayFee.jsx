...
export default function PayFee({
  ...
  const [payment, setPayment] = useState(""); // ✅ changed to string
  ...
  useEffect(() => {
    if (fee) {
      ...
      setPayment(""); // ✅ start empty instead of 0
  ...
  const hasChanges =
    totalFee !== fee.Amount_Due || Number(payment) > 0;
  ...
  const handleSubmit = async () => {
    ...
    await onSubmit({
      Id: fee.Id,
      NewAmount: totalFee,
      PaymentAmount: paymentValue, // ✅ converted here
  ...
  const handleFull = () => {
    ...
    setTotalFee(newFee);
    setPayment(String(newBalance)); // ✅ string
    setSelectedOption("full");
  };
  ...
  const handleHalf = () => {
    ...
    setTotalFee(newFee);
    setPayment(String(newBalance > 0 ? newBalance : 0)); // ✅ string
